...
@inject(BannerService, BizProductsService, UtilitiesJS, Router,LogService)
export class EditBanner {
    ...
    constructor(bannerService, bizProductsService, utilitiesJS, router,logService) {
    ...
    SubmitBanner() {
        ...
        this.bannerService.UpdateBanner(jsonToPost)
            .then((data) => {
                if (data.Result === true) {
                    this.logService.InsertAdminCPLog("Affiliate | EditBanner",data.Result, JSON.stringify(jsonToPost));
                    toastr.success('Cập nhật banner thành công!', "Banner");
                    location.href = `#/MOTMenus/BannerMng`;
                    return true;
                } else {
                    this.logService.InsertAdminCPLog("Affiliate | EditBanner","Lỗi!","Không thể cập nhật banner");
