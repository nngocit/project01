...
export class EditCampaign {
    ...
    editBusiness() {
        ...
        this.bizProductsService.SubmitBusinessCampaign(jsonToPost).then((data) => {
            if (data.Result == true) {
                this.logService.InsertAdminCPLog("Affiliate | Quan ly doi Tac | updateBusiness",data.Result, JSON.stringify(jsonToPost));
                toastr.success('Cập nhật đối tác thành công!', "Chương trình");
                window.location = "#MOTMenus/CampaignMng";

                return true;
            } else {
                this.logService.InsertAdminCPLog("Affiliate | Quan ly doi Tac | updateBusiness","false", JSON.stringify(jsonToPost));
