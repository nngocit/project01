...
@inject(CoBrandService,LogService)
export class CoBrand {
    ...
    constructor(coBrandService,logService) {
    ...
    async InsertCoBrandItem() {
        ...
        if (response != null) {
            if (response.Result == true) {
                this.ClearForm();
                this.logService.InsertAdminCPLog("Affiliate | Co-Brand | InsertCoBrandItem",response.Result, JSON.stringify(jsonToPost));    
                toastr.success("Thêm mới Số điện thoại thành công!", "Success");
            } else {
                this.logService.InsertAdminCPLog("Affiliate | Co-Brand | InsertCoBrandItem","false", JSON.stringify(jsonToPost));    
