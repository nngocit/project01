...
@inject(BannerService, BizProductsService, UtilitiesJS,LogService)
export class AddBanner {
    ...
    constructor(bannerService, bizProductsService, utilitiesJS,logService) {
    ...
    SubmitBanner() {
        ...
        this.bannerService.InsertBanner(jsonToPost).then((data) => {
            if (data.Result == true) {
                this.InitBanner();
                this.logService.InsertAdminCPLog("Affiliate | InsertBanner",data.Result, JSON.stringify(jsonToPost));
                toastr.success('Tạo mới Banner thành công!', "Banner");
                $('#BannerDescription').trumbowyg('empty');
                return true;
            } else {
                this.logService.InsertAdminCPLog("Affiliate | InsertBanner","false", JSON.stringify(jsonToPost));
