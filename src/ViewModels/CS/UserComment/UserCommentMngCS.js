...
import 'select2';
import {
    UserRegisterCommentMngCS
} from './UserRegisterCommentMngCS';
import {
    UserUpdateCommentMngCS
} from './UserUpdateCommentMngCS';
import {
    UserHistoryCommentMngCS
} from './UserHistoryCommentMngCS';
import {
    DialogService
} from 'aurelia-dialog';
...
export class UserCommentMngCS {
    ...
    activate() {
        ...
        return Promise.all([
            ...
            //   console.log(JSON.stringify(this.ListPhongBanRole));
    ...
    attached() {
        ...
        //             if (data.Result == true) {
        //                 this.logService.InsertAdminCPLog("VTA APP | CS | QuanLyUser | Autologout", data.Result, data);
        ...
        try {
            return this.commentService.SearchUserBy(this.strQuery).then((data) => {
                ...
                if (data.status === 200 && data.data !== "NOT_FOUND") {
                    ...
                    this.ListUsers = data.data.filter(x => x.Nhom === "CS");
    ...
    ViewdlgUserRegister(item) {
        ...
        this.dialogService.open({
            viewModel: UserRegisterCommentMngCS,
            model: item
        }).then((result) => {
    ...
    ViewdlgUserUpdate(item) {
        ...
        this.dialogService.open({
            viewModel: UserUpdateCommentMngCS,
            model: item
        }).then((result) => {
    ...
    ViewdlgUserHistory(item) {
        ...
        this.dialogService.open({
            viewModel: UserHistoryCommentMngCS,
            model: item
        }).then((result) => {
            // console.log('callback update');
            this.commentService.SearchUserBy(this.strQuery).then((data) => {
