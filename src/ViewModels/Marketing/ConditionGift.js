...
@inject(QuaySoService, QuaySoPermission,LogService)
export class ConditionGift {
    ...
    constructor(quaySoService, quaySoPermission,logService) {
    ...
    activate() {
        return Promise.all([this.quaySoService.GetListQuaySoGift(),
            ...
            this.ListBranchs = rs[3].Data;

            //console.log(this.ListConditions);
    ...
    attached() {
        ...
        $('#filterByCondition').select2().val(this.filterCondition);
        $('#filterByCondition').select2({
            placeholder: "- Chọn Điều kiện quay -",
            allowClear: true
        }).on('change', () => {
            this.filterCondition = $('#filterByCondition').val();
        });

        $('#filterByGift').select2().val(this.filterGift);
        $('#filterByGift').select2({
            placeholder: "- Chọn Quà tặng -",
            allowClear: true
        }).on('change', () => {
            this.filterGift = $('#filterByGift').val();
        });
    }
    ...
    SubmitConditionGift() {
        ...
        this.quaySoService.InsertOrUpdateQuaySoConditionGift(jsonToPost).then((data) => {
            if (data.Result == true) {
                ...
                this.logService.InsertAdminCPLog("Marketing | Cau Hinh Tuong Ung Dieu Kien Quay",this.isEdit === false ? 'Tạo mới Quà tặng tương ứng Điều kiện quay thành công!' : 'Cập nhật Quà tặng tương ứng Điều kiện quay thành công', JSON.stringify( this.ListConditionGifts));
    ...
    LoadDDLAddEditQuaTang() {
        $('#ddlAddEditQuaTang').select2().val(this.currentConditionGift.GiftId);
        $('#ddlAddEditQuaTang').select2({
            placeholder: "- Chọn Qùa tặng -",
            allowClear: true
        }).on('change', () => {
            this.currentConditionGift.GiftId = $('#ddlAddEditQuaTang').val();
        });
    }
...
export class MergeBrachAndConditionValueConverter {
    toView(condition, ListBranchs) {
        if (condition != null) {
            ...
            //return condition.CampaignName + "- " + branch[0].BranchsShortAdd;
                return condition.CampaignName + " - " + condition.Id + " - " + branch[0].BranchsCodePos;
        }
        // return "[Tất cả chi nhánh" + "] - Từ " + moment(condition.DateFrom).format('DD/MM/YYYY') + " đến " + moment(condition.DateTo).format('DD/MM/YYYY');
        return condition.CampaignName + "- Tất cả chi nhánh";
    }
}
    ...
    toView(Id, ListConditions, ListBranchs) {
        if (Id !== "") {
            ...
            if (condition.length > 0) {
                ...
                if (branch.length > 0)
                    return branch[0].BranchsCodePos + "- " + branch[0].BranchsAdd;
            }
        }
...
export class CampaignNameConditionValueConverter {
    toView(Id, ListConditions) {
        if (Id !== "") {
            var condition = ListConditions.filter(x => x.Id !== null && x.Id === Id);
            //console.log(condition);
            if (condition != null)
                return condition[0].CampaignName;
        }
        return "N/A";
    }
}
