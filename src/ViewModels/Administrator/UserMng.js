...
@inject(UserService, BizProductsService, UtilitiesJS, LogService)
export class UserMng {
    ...
    constructor(userService, bizProductsService, utilitiesJS, logService) {
    ...
    SubmitUser() {
        ...
        this.userService.SubmitUser(jsonToPost).then((data) => {
            if (data.Result == true) {
                ...
                this.logService.InsertAdminCPLog("Administrator | Quan ly User", this.isEdit === false ? 'Tạo mới user thành công!' : 'Cập nhật user thành công!', JSON.stringify(jsonToPost));
                toastr.success(this.isEdit === false ? 'Tạo mới user thành công!' : 'Cập nhật user thành công!', "Thông báo");
                return true;
            } else {
                this.logService.InsertAdminCPLog("Administrator | Quan ly User", 'Lỗi', JSON.stringify(jsonToPost));
    ...
    ResetPassword(user) {
        ...
        this.userService.ResetPassword(jsonToPost).then((data) => {
            if (data.Result == true) {
                ...
                this.logService.InsertAdminCPLog("Administrator | Quan ly User", "Reset mật khẩu user thành công! Mật khẩu mới là \'123456\'", JSON.stringify(jsonToPost));
                toastr.success('Reset mật khẩu user thành công! Mật khẩu mới là \'123456\'', "Thông báo");
                return true;
            } else {
                this.logService.InsertAdminCPLog("Administrator | Quan ly User", "Không thể reset mật khẩu user. Xin thử lại!", JSON.stringify(jsonToPost));
